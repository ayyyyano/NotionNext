...
const ThemeColorSwitch = ({
  ...
  const applyColor = (nextHue) => {
    const root =
      document.getElementById('theme-fuwari') ||
  ...
  useEffect(() => {
    const stored =
      localStorage.getItem(STORAGE_HUE_KEY)

    const initialHue = stored
      ? parseInt(stored, 10)
      : defaultHue
  ...
  const handleSelect = nextHue => {
    ...
    const nextColor = applyColor(nextHue)
