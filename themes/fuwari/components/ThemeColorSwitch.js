...
const ThemeColorSwitch = ({ onColorChange }) => {
  ...
  const applyColor = (nextHue) => {
    const root = document.getElementById('theme-fuwari') || document.documentElement
  ...
  useEffect(() => {
    ...
    if (!stored) return
    const initialHue = parseInt(stored, 10)
    setHue(initialHue)
    // 延迟确保 DOM 已渲染
    requestAnimationFrame(() => applyColor(initialHue))
  }, [])
  ...
  const handleSelect = (nextHue) => {
    ...
    applyColor(nextHue)
    onColorChange?.(hslToHex(nextHue, 85, 62))
  }

  if (!enabled) return null

  const copyHex = async () => await navigator.clipboard.writeText(color)
    ...
    <section className='fuwari-theme-panel p-4'>
      <div className='flex items-center justify-between mb-3'>
        ...
        <div className='flex items-center gap-2'>
          <button type='button' onClick={() => handleSelect(defaultHue)} className='fuwari-tool-btn w-8 h-8' title='Reset default hue'>
            <i className='fas fa-rotate-left text-xs' />
          </button>
          <button type='button' onClick={copyHex} className='px-2 h-8 rounded-md bg-[var(--fuwari-bg-soft)] border border-[var(--fuwari-border)] text-[var(--fuwari-primary)] font-bold'>
            {hue}
          </button>
        </div>
      </div>
      <div className='fuwari-hue-wrap'>
        <input type='range' min='0' max='360' step='5' value={hue} onChange={e => handleSelect(parseInt(e.target.value, 10))} className='fuwari-hue-slider' />
      </div>
